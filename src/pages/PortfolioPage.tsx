 ...
 * Portfolio Page
 *
 * Expanded gallery view with full artwork collection.
 * Features:
 * - All artworks (not just featured)
...
function ArtworkCard({ artwork, index, onClick }: ArtworkCardProps) {
  ...
  return (
    <motion.article
      ...
      transition={{ duration: durations.smooth, delay: index * 0.05 }}
...
export function PortfolioPage() {
  ...
  const filteredArtworks = activeCategory
    ? artworks.filter(a => a.collection === activeCategory)
    : artworks;
  ...
  return (
    <section className="relative py-32 sm:py-40 lg:py-48 bg-luxury-charcoal min-h-screen overflow-hidden">
      {/* Background elements */}
      ...
      <Container size="xl">
        ...
        <ScrollReveal direction="up" className="text-center mb-12 sm:mb-16">
          ...
          <Text size="lg" color="muted" align="center" className="max-w-2xl mx-auto">
            Explore the full collection of SoundBYTE artworks, from bespoke commissions
            to collaborative masterpieces. Each piece tells a unique story through the
            visual language of sound.
          </Text>
        </ScrollReveal>
        ...
        <ScrollReveal direction="up" className="mb-12">
          <div className="flex flex-wrap justify-center gap-3">
            ...
            {categories.map((cat) => (
              ...
              >
                {cat.name} ({artworks.filter(a => a.collection === cat.id).length})
              </MagneticButton>
            ))}
          </div>
        </ScrollReveal>
        ...
        {filteredArtworks.length === 0 && (
          <div className="text-center py-20">
            <p className="text-white/60">No artworks found in this category.</p>
          </div>
        )}
